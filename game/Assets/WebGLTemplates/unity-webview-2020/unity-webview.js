var unityWebView = {
  ...
  loadURL: function (name, url) {
    let iframe = this.iframe(name).attr("loaded", "false");
  ...
  evaluateJS: function (name, js) {
    ...
    const contentHost = new URL($iframe.attr("data-url") || location.href)
      .hostname;
    const currentHost = location.hostname;

    let safeExec =
      contentHost.includes(currentHost) || currentHost.includes(contentHost);
    ...
    if (safeExec) {
      if ($iframe.attr("loaded") === "true") {
        evaluate($iframe[0].contentWindow);
      } else {
        $iframe.on("load", function () {
          evaluate($(this)[0].contentWindow);
        });
      }
    }
  },
  ...
  iframe: (name) => {
    return $("iframe").attr("id", "webview_" + name);
  },
};
